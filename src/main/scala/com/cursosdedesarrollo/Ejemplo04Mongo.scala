...
object Ejemplo04Mongo{
  ...
  def main(args: Array[String]): Unit = {
    ...
    var insertObservable: Observable[Completed] = collection2.insertOne(document)
    ...
    collection2.find.subscribe(new Observer[Document] {
      ...
      override def onComplete(): Unit = {
        ...
        collection2.find.first().subscribe(new Observer[Document] {
        ...
        insertObservable = collection2.insertMany(documents)

        val insertAndCount = for {
          insertResult <- insertObservable
          countResult <- collection2.countDocuments()
        } yield countResult
        ...
        collection2.find(equal("i", 71)).first().subscribe(new Observer[Document] {
          override def onNext(result: Document): Unit = println("Documento: "+result)
          override def onError(e: Throwable): Unit = e.printStackTrace()
          ...
          override def onComplete(): Unit = println("Find Completado")
            ...
            // Filtrado
            collection2.find(gt("i", 50)).subscribe(new Observer[Document] {
              override def onNext(result: Document): Unit = println("Documento: "+result)
              override def onError(e: Throwable): Unit = e.printStackTrace()
              override def onComplete(): Unit = println("Listado Impreso")
            })
            // Filtrado por rango
            collection2.find(and(gte("i", 50), lte("i", 100))).subscribe(new Observer[Document] {
              override def onNext(result: Document): Unit = println("Documento: "+result)
              override def onError(e: Throwable): Unit = e.printStackTrace()
              override def onComplete(): Unit = println("Listado Impreso")
            })
            ...
            // Ordenado
            collection2.find(exists("i")).sort(descending("i")).first().subscribe(new Observer[Document] {
              override def onNext(result: Document): Unit = println("Documento: "+result)
              override def onError(e: Throwable): Unit = e.printStackTrace()
              override def onComplete(): Unit = println("Listado Impreso")
            })
            ...
            // Proyección
            collection2.find().projection(excludeId()).first().subscribe(new Observer[Document] {
              override def onNext(result: Document): Unit = println("Documento: "+result)
              override def onError(e: Throwable): Unit = e.printStackTrace()
              override def onComplete(): Unit = println("Listado Impreso")
            })
            ...
            // Update One
            collection2.updateOne(equal("i", 10), set("i", 110)).subscribe(new Observer[UpdateResult] {
            ...
            // Delete One
            collection2.deleteOne(equal("i", 110)).subscribe(new Observer[DeleteResult] {
            ...
            // Delete Many
            collection2.deleteMany(gte("i", 100)).subscribe(new Observer[DeleteResult] {
              override def onNext(result: DeleteResult): Unit = println("Resultado Delete:" +result)
            ...
            collection2.drop().subscribe(new Observer[Completed] {
              ...
              override def onComplete(): Unit = {
                ...
                collection2.bulkWrite(writes).subscribe(new Observer[BulkWriteResult] {
                  ...
                  override def onComplete(): Unit ={
                    ...
                    collection2.drop().subscribe(new Observer[Completed] {
                      ...
                      override def onComplete(): Unit = {
                        println("Borrado completado")
                        collection2.bulkWrite(writes, BulkWriteOptions().ordered(false)).subscribe(new Observer[BulkWriteResult] {
